...
function addGamesToPage(games) {
    ...
    for(var i = 0; i < games.length; i++){
        ...
        game_card.innerHTML = `<div>${game.name}</div>
        <p>${game.description}</p>
        <div>Pledged: $${game.pledged}</div>
        <div>Goal: $${game.goal}</div>
        <div>Amount of Backers: ${game.backers}</div>
...
raisedCard.innerHTML =`<div> $${totalRaised.toLocaleString('en-US')} </div>`;
...
let numberOfUnfunded = listOfNotReached.length;


// create a string that explains the number of unfunded games using the ternary operator
const displayString = `A total of $${totalContributions.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games.
Currently, ${numberOfUnfunded == 1 ? "1" : numberOfUnfunded} ${numberOfUnfunded == 1 ? "game remains" : "games remain"} 
...
pTop.innerHTML = firstGame.name;
firstGameContainer.append(pTop);

// do the same for the runner up item
const pRunnerUp = document.createElement("p");
pRunnerUp.innerHTML = secondGame.name;
secondGameContainer.append(pRunnerUp);